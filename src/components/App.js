...
function App() {
    ...
    const [devINR, setDevINR] = useState("");
    // Modal States
    ...
    // Moonbeam conversion rate
    
    async function fetchDevUSD() {
        const COINBASE_BASE_URL = "https://api.coinbase.com/v2";
        const res = await fetch(`${COINBASE_BASE_URL}/prices/GLM-USD/buy`);
        const data = await res.json();
        setDevUSD(data.data.amount);
    }

    async function fetchDevINR() {
        const COINBASE_BASE_URL = "https://api.coinbase.com/v2";
        const res = await fetch(`${COINBASE_BASE_URL}/prices/GLM-INR/buy`) ;
        const data = await res.json();
        setDevINR(data.data.amount);
    }

    const loadBlockchainData = useCallback(async () => {
        var accounts;

        async function addMoonbaseTestnetNetwork() {
            try {
                await ethereum.request({
                    method: "wallet_switchEthereumChain",
                    params: [{ chainId: "0x507" }], // Hexadecimal version of 1287, prefixed with 0x
                });
            } catch (error) {
                if (error.code === 4902) {
                    try {
                        await ethereum.request({
                            ...
                            params: [
                                {
                                    chainId: "0x507", // Hexadecimal version of 1287, prefixed with 0x
        ...
        if (ethereum) {
            ...
            await addMoonbaseTestnetNetwork();
        } else if (ethereum && (await ethereum.request({ method: "net_version" })) !== "1287") {
            window.web3 = new Web3(window.web3.currentProvider);
            await addMoonbaseTestnetNetwork();
        } else {
            // const web3 = new Web3(Web3.givenProvider || "http://localhost:7545");
            window.web3 = new Web3(new Web3.providers.HttpProvider("wss://frag-moonbase-relay-rpc-ws.g.moonbase.moonbeam.network"));
            window.alert(
