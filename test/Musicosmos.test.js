...
const Musicosmos = artifacts.require("./Musicosmos.sol");

require("chai").use(require("chai-as-promised")).should();

contract("Musicosmos", ([deployer, seller, buyer, buyer2]) => {
	let contract;

	before(async () => {
		contract = await Musicosmos.deployed();
	});
	...
	describe("deployment", async () => {
		...
		it("has a name", async () => {
			...
			assert.equal(name, "Musicosmos");
		});

		it("has a symbol", async () => {
			const symbol = await contract.symbol();
			assert.equal(symbol, "MSC");
		});
	});
	...
	describe("songs", async () => {
		...
		describe("purchase", async () => {
			...
			it("purchases song when artist and owner are different", async () => {
				...
				let newMusicosmosMainBalance = await web3.eth.getBalance(musicosmosMainAddress);
				newMusicosmosMainBalance = BigNumber(newMusicosmosMainBalance);
				...
				let expectedPlatformBalance = oldMusicosmosMainBalance.plus(priceTransferredToPlatform);
				assert.equal(newMusicosmosMainBalance.toString(), expectedPlatformBalance.toString());
