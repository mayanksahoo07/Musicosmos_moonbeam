...
contract Musicosmos is ERC721, ReentrancyGuard {
    using SafeMath for uint256;

    uint256 public songsCount = 0;
    address MusicosmosMain = 0x8FBC5c40Cf9F8336a3909b7278Fd691Dbe03dd79;

    constructor() ERC721("Musicosmos", "MSC") {}
    ...
    function purchaseSong(uint256 _id) public payable nonReentrant {
        ...
        // Require that there is enough dev provided for the transaction
        require(msg.value >= _song.price, "Not enough dev provided");
        ...
        if (_song.currentOwnerAddress == _song.artistAddress) {
            // Transfer all dev to artist whenever the first transaction for the NFT happens. This is because the artist is the owner after creating the NFT.
            payable(_song.currentOwnerAddress).transfer(msg.value);
            emit FundsTransferredToArtist(msg.value);
        } else {
            // Pay the seller by sending 94% dev
            ...
            // Pay the platform fee by sending the remaining 1% dev
            uint256 _platformFee = ((msg.value).mul(1)).div(100);
            payable(MusicosmosMain).transfer(_platformFee);
